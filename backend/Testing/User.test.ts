...
test('Get User object works properly', async () => {
    const user: User = User.getUserObject('a.b@c.de', true, 'secure');

    expect(user.getMail()).toEqual('a.b@c.de');
    expect(user.getAdmin()).toEqual(true);
    expect(user.getPassword()).toEqual('secure');
});
